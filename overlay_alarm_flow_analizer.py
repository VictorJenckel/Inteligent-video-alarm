...
class ZoneMonitor:
    ...
    def draw(self, image, is_config_mode=False):
        ...
        bw, bh = 10, 50
        bx, by = x - 15, y
        cv2.rectangle(image, (bx, by), (bx+bw, by+bh), (50, 50, 50), -1)
        fh = int(min(self.current_score / self.max_display_score, 1.0) * bh)
        cv2.rectangle(image, (bx, by+bh-fh), (bx+bw, by+bh), COLOR_OK, -1)
        lr = min(self.alarm_limit / self.max_display_score, 1.0)
        ly = int(by + bh - lr * bh)
        cv2.line(image, (bx-2, ly), (bx+bw+2, ly), (0, 255, 255), 2)
        status = "ALARM" if self.alarm_latched else "OK"
        cv2.putText(image, f"{self.current_score}|{status}",
                    (bx, by-8), cv2.FONT_HERSHEY_SIMPLEX, 0.38, COLOR_OK, 1)

        # Info Flow
        fr      = self._flow_analyzer.latest_result
        rolling = fr.get('flow_rolling', 0.0)
        thresh  = self.flow_threshold / 100.0
        fcol    = (0, 80, 255) if fr.get('is_motion_alarm') else (160, 160, 160)
        flabel  = "FLOW" if self.flow_enabled else "FLOW:OFF"
        cv2.putText(image, f"{flabel} {rolling:.2f}/{thresh:.2f}",
                    (bx, by+bh+14), cv2.FONT_HERSHEY_SIMPLEX, 0.34, fcol, 1)
...
class App:
    ...
    def refresh_trackbars(self):
        ...
        if cv2.getWindowProperty(self.ctrl_window, cv2.WND_PROP_VISIBLE) < 1:
            ...
            cv2.resizeWindow(self.ctrl_window, 420, 200)
